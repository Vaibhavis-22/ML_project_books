...
def get_content_recommendations(book_title, vector_df, spark, top_n=10):
    try:
        ...
        # Load LSH model
        lsh_model = BucketedRandomProjectionLSHModel.load("models/lsh_pca_model")

        # Get vector for the given book title
        book_vector = vector_df.filter(col("title") == book_title).select("title", "final_features")
        ...
        key_vector = book_vector.first()["final_features"]
